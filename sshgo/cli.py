...
class SSHGoCLI:
    ...
    def connect_by_name(self, name: str):
        ...
        # Для RDP подключений показываем сообщение, что подключение запущено в фоне
        if server.type.lower() == 'rdp':
            print_colored(Colors.GREEN, f"✅ RDP подключение запущено в фоновом режиме")
            print_colored(Colors.BLUE, "💡 Консоль свободна для дальнейшей работы")
    ...
    def show_menu(self):
        ...
        if server:
            ...
            if return_code != 0:
                print_colored(Colors.RED, f"❌ Ошибка подключения (код: {return_code})")
                # Не ждем Enter - сразу возвращаем управление даже при ошибке
            else:
                # Для RDP подключений показываем сообщение
                if server.type.lower() == 'rdp':
                    print_colored(Colors.GREEN, f"✅ RDP подключение запущено в фоновом режиме")
                    print_colored(Colors.BLUE, "💡 Консоль свободна для дальнейшей работы")
                # Для SSH просто возвращаем управление (пользователь уже вышел из сессии)
                # Не ждем Enter - сразу возвращаем управление
